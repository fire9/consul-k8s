...
func TestRun_FlagValidation(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			ExpErr: "if -enable-acl-replication is true, -acl-replication-token or the environment variable ACL_REPLICATION_TOKEN must be set",
		},
	}
	...
	for _, c := range cases {
		t.Run(c.TokenFlag, func(t *testing.T) {
			...
			cmdArgs := []string{
				"-k8s-namespace=" + ns,
				"-expected-replicas=1",
				"-acl-replication-token", bootToken,
...
// Test that the ACL replication token can be passed via flag or environment
// variable.
func TestRun_ACLReplicationTokenFlagEnvVar(t *testing.T) {
	t.Parallel()
	bootToken := "aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee"
	...
	cases := map[string]struct {
		...
		"flag": {
			Flags: []string{"-acl-replication-token", bootToken},
		},
		"env var": {
			EnvVars: map[string]string{"ACL_REPLICATION_TOKEN": bootToken},
		},
		// Test that the flag overrides the env var by passing an env var token
		// that would fail if it were used.
		"flag overrides env var": {
			Flags:   []string{"-acl-replication-token", bootToken},
			EnvVars: map[string]string{"ACL_REPLICATION_TOKEN": "this-would-fail"},
		},
	}
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			// We only create a client token since that's enough to test that
			// the acl replication token was passed successfully.
			cmdArgs := []string{
				"-k8s-namespace=" + ns,
				"-expected-replicas=1",
				"-enable-acl-replication",
			...
			// Test that the client policy was created successfully.
			retry.Run(t, func(r *retry.R) {
				policy := policyExists(r, "client-token-dc2", consul)
				require.Equal(r, []string{"dc2"}, policy.Datacenters)
			})
		})
	}
}
...
func TestRun_ACLReplicationTokenValid(t *testing.T) {
	...
	secondaryCmdArgs := []string{
		...
		"-resource-prefix=" + resourcePrefix,
		"-enable-acl-replication",
		"-acl-replication-token", aclReplicationToken,
		"-create-client-token",
